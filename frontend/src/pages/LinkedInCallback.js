...
export default function LinkedInCallback() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!code || !state) {
        setError('Missing connection details from LinkedIn. Please try connecting again.');
        setStatus('error');
        return;
      }

      try {
        const res = await fetch(`${API}/api/linkedin/oauth-callback`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ code, state })
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.detail || 'Could not connect LinkedIn');
        ...
        {status === 'success' && (
          <>
            <div className="success-msg">
              {name ? `Connected as ${name} on LinkedIn.` : 'LinkedIn connected.'}
            </div>
