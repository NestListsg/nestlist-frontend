...
export default function FacebookCallback() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!code || !state) {
        setError('Missing connection details from Facebook. Please try connecting again.');
        setStatus('error');
        return;
      }

      try {
        const res = await fetch(`${API}/api/facebook/oauth-callback`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ code, state })
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.detail || 'Could not connect Facebook');
        ...
        {status === 'success' && (
          <>
            <div className="success-msg">
              {pageName ? `Connected to ${pageName} on Facebook.` : 'Facebook connected.'}
            </div>
