          ...
          <div ref={listRef} style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {messages.length === 0 && (
              <div style={{ color: 'rgba(248,244,236,0.5)', fontSize: '12.5px', lineHeight: '1.6' }}>
                Ask anything property-related — land area conversions, PSF, stamp duty, mortgage estimates, current market rates, or a question about a listing you have open.
              </div>
            )}
            {messages.map((m, i) => (
              <div key={i} style={{
            ...
            {loading && (
              <div style={{ alignSelf: 'flex-start', color: 'rgba(248,244,236,0.5)', fontSize: '12.5px', fontStyle: 'italic' }}>
                Thinking...
              </div>
            )}
            {error && (
